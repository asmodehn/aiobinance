...
@pytest.mark.vcr(filter_headers=['X-MBX-APIKEY'], filter_query_parameters=["timestamp", "signature"])
def test_balance_from_binance(keyfile):
    """ get binance balances"""

    account = aiobinance.binance.balance_from_binance(key=keyfile.get('key'), secret=keyfile.get('secret'))
...
def test_trades_from_binance(keyfile):
    ...
    trades = aiobinance.binance.trades_from_binance("COTIBNB", start_time=start_time, end_time=end_time,
    ...
    first = trades[0]
    assert datetime.fromtimestamp(start_time / 1000, tz=timezone.utc) < first.time < datetime.fromtimestamp(end_time / 1000, tz=timezone.utc)
    assert first.time == datetime(year=2020, month=8, day=27, hour=10, minute=57, second=44, microsecond=478000, tzinfo=timezone.utc)
...
def test_price_from_binance(keyfile):
    ...
    start_time = 1598524340551
    end_time = start_time + 24 * 3_600_000
    ohlcv = aiobinance.binance.price_from_binance("COTIBNB", start_time=start_time, end_time=end_time)
    ...
    first = ohlcv[0]
    assert first.open_time == datetime(year=2020, month=8, day=27, hour=10, minute=33, tzinfo=timezone.utc)
    ...
    assert first.close_time == datetime(year=2020, month=8, day=27, hour=10, minute=35, second=59, microsecond=999000, tzinfo=timezone.utc)
