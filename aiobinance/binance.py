...
def optional_key_load(key: Optional[str] =None, secret: Optional[str]=None): # TODO : type for keystruct
...
def balance_from_binance(*, key: str, secret: str) -> Account:
    api = BinanceRaw(API_KEY=key,
                     API_SECRET=secret)  # we need private requests here !
...
@require_auth()
def trades_from_binance(symbol: str, *, start_time: int, end_time: int, key: str, secret:str) -> TradeFrame:
    api = BinanceRaw(API_KEY=key,
                     API_SECRET=secret)  # we need private requests here !

    res = api.call_api(command='myTrades', symbol=symbol, startTime=start_time, endTime=end_time, limit=1000)
...
def price_from_binance(symbol: str, *, start_time: int, end_time: int) -> OHLCV:
    ...
    res = api.call_api(command='klines', symbol=symbol, interval=interval, startTime=start_time, endTime=end_time,
                       limit=1000)
...
if __name__ == "__main__":
    ...
    print(trades_from_binance("COTIBNB", start_time=1598524340551, end_time=1598893442120))
    print(price_from_binance("COTIBNB", start_time=1598524340551, end_time=1598893442120))
